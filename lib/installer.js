...
export const DEFAULT_SOURCE = 'github:shaomingbo/dsh-open-in-editor#v0.2.0'
...
export async function updateProfile({
  ...
  const next = formatManifest(manifest, original)
  ...
  try {
    await installDependencies(profileDirectory)
  } catch (cause) {
    if (changed) await atomicWrite(manifestPath, original)
    throw new Error(`Dependency installation failed; package.json was restored. ${cause.message}`)
  }
