window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Vendored from @deepseek-ai/dsh-util-workspace-path@0.1.2-alpha.2, the successor of
    // the client helper @deepseek-ai/dsh-client-runtime used to export before DSH removed
    // that package. Neither row exists in the module table, so the browser-safe helper
    // travels with the bundle instead of being required.
...
`
    ...
    function createCatalog(connection) {
      ...
      const load = (refresh = false) => {
        ...
        inflight = connection.rpc.call(API_CHANNEL, 'open-in-editor/describe', { refresh })
          .catch(() => connection.rpc.call(CHANNEL, 'describe', { refresh })).then((result) => {
    ...
    async function requestOpen(connection, path, editor) {
      ...
      const result = await connection.rpc.call(API_CHANNEL, 'open-in-editor/open', payload)
        .catch(() => connection.rpc.call(CHANNEL, 'open', payload))
      if (!result.ok) throw new Error(result.error.message)
      return result.value
    }
    ...
    function ProducedFiles({ matched: paths, sessionId, useSessions, inputActions, connection, catalog, t }) {
      ...
      const copyPath = React.useCallback(async (path) => {
        try {
          ...
          inputActions?.notify?.('info', t('copied', { name: fileName(path) }))
          closeMenu(true)
        } catch (cause) {
