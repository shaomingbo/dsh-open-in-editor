window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      settingsIntro: 'Choose which local macOS application opens produced files. The arrow beside a file can choose another IDE for one open.',
      ...
      choose: 'Choose an application for {name}',
      menu: 'Open {name} with',
      more: '+{count} more',
    ...
    const zh = {
      ...
      settingsIntro: '选择用于打开产物文件的本地 macOS 应用。文件旁的箭头可临时选择其他 IDE，不会修改默认值。',
      system: '系统默认',
      installed: '已安装',
      unavailable: '未安装',
      loading: '正在检测应用…',
...
.dsh-open-in-editor-main:focus-visible,.dsh-open-in-editor-arrow:focus-visible,.dsh-open-in-editor-menu-item:focus-visible,.dsh-open-in-editor-option:focus-visible,.dsh-open-in-editor-refresh:focus-visible{outline:2px solid var(--dsw-alias-interactive-border-focus,currentColor);outline-offset:-2px}
...
.dsh-open-in-editor-more{color:var(--dsw-alias-label-tertiary,var(--dsw-alias-label-secondary));font-size:12px;line-height:28px}
...
`
    ...
    function OpenMenu({ menu, catalog, pending, onOpen, onClose, t }) {
    ...
    function ProducedFiles({ matched: paths, sessionId, useSessions, inputActions, connection, catalog, t }) {
      ...
      const visible = paths.slice(0, MAX_VISIBLE_FILES)
      return h('div', { className: 'dsh-open-in-editor-row' },
        ...
        paths.length > MAX_VISIBLE_FILES
          ? h('span', { className: 'dsh-open-in-editor-more' }, t('more', { count: paths.length - MAX_VISIBLE_FILES }))
          : null,
